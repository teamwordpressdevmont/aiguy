...
<header class="bg-white shadow-sm">
    <div class="container mx-auto py-5  px-4">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900">Edit Course</h1>
    </div>
</header>
        ...
        <div class="sm:col-span-4 mb-5">
            ...
            <div class="mt-2">
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                    <input type="text" name="name" id="name" value="{{ $course->name }}" class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" placeholder="janesmith">
                </div>
            </div>
        </div>          
                ...
                <select name="categoryCourses[]" multiple class="col-start-1 row-start-1 w-full appearance-none rounded-md bg-white py-1.5 pr-8 pl-3 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                    ...
                    @foreach ($categoryCourses as $category)
                        <option value="{{ $category->id }}" 
                            {{ $course->categories ? (in_array($category->id, $course->categories->pluck('id')->toArray()) ? 'selected' : '') : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
